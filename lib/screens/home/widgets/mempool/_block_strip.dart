...
class _BlockStripState extends State<BlockStrip>
  ...
  //   - first paint (post-frame, once the viewport has a width),
  ...
  void initState() {
    ...
    // to after the first layout so viewportDimension is available.
    WidgetsBinding.instance.addPostFrameCallback((_) => _recenterDivider());
  }
  ...
  @override
  void didPopNext() => _recenterDivider();

  @override
  void didChangeAppLifecycleState(AppLifecycleState state) {
    if (state == AppLifecycleState.resumed) _recenterDivider();
  }

  void _recenterDivider() {
    if (!mounted || !_ctrl.hasClients) return;
    if (_ctrl.position.viewportDimension <= 0) return;
    final projectedShown = widget.snapshot.projected.length.clamp(0, 8);
